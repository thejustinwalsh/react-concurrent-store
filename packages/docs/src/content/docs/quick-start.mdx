...
A dispatch inside `startTransition` is one no component has to show yet. A
dispatch outside one has to be visible now, so it applies to what is on screen
rather than to the state the Transition is still waiting for.

```tsx
// Slow: the old screen stays up until this is ready.
startTransition(() => store.dispatch({ type: "navigate", to: "/profile" }));

// Blocking: visible immediately, on the page they are still looking at.
